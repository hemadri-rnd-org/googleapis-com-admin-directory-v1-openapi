...
func Directory_resources_buildings_getHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateDirectory_resources_buildings_getTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_admin_directory_v1_customer_customer_resources_buildings_buildingId",
		mcp.WithDescription("Retrieves a building."),
		...
		mcp.WithString("buildingId", mcp.Required(), mcp.Description("The unique ID of the building to retrieve.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Directory_resources_buildings_getHandler(cfg),
	}
}
