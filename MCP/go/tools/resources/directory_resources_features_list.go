...
func Directory_resources_features_listHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/admin/directory/v1/customer/%s/resources/features%s", cfg.BaseURL, customer, queryString)
...
func CreateDirectory_resources_features_listTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_admin_directory_v1_customer_customer_resources_features",
		mcp.WithDescription("Retrieves a list of features for an account."),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Directory_resources_features_listHandler(cfg),
	}
}
