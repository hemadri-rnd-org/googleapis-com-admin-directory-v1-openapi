...
func Directory_resources_buildings_deleteHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateDirectory_resources_buildings_deleteTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_admin_directory_v1_customer_customer_resources_buildings_buildingId",
		mcp.WithDescription("Deletes a building."),
		...
		mcp.WithString("buildingId", mcp.Required(), mcp.Description("The id of the building to delete.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Directory_resources_buildings_deleteHandler(cfg),
	}
}
