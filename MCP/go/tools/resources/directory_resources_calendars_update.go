...
func Directory_resources_calendars_updateHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateDirectory_resources_calendars_updateTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("put_admin_directory_v1_customer_customer_resources_calendars_calendarResourceId",
		mcp.WithDescription("Updates a calendar resource. This method supports patch semantics, meaning you only need to include the fields you wish to update. Fields that are not present in the request will be preserved."),
		...
		mcp.WithString("calendarResourceId", mcp.Required(), mcp.Description("The unique ID of the calendar resource to update.")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Directory_resources_calendars_updateHandler(cfg),
	}
}
