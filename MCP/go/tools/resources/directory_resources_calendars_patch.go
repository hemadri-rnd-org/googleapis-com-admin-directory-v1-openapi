...
func Directory_resources_calendars_patchHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateDirectory_resources_calendars_patchTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("patch_admin_directory_v1_customer_customer_resources_calendars_calendarResourceId",
		mcp.WithDescription("Patches a calendar resource."),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Directory_resources_calendars_patchHandler(cfg),
	}
}
