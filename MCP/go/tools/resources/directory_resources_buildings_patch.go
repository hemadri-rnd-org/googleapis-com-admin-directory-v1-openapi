...
func Directory_resources_buildings_patchHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateDirectory_resources_buildings_patchTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("patch_admin_directory_v1_customer_customer_resources_buildings_buildingId",
		mcp.WithDescription("Patches a building."),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Directory_resources_buildings_patchHandler(cfg),
	}
}
