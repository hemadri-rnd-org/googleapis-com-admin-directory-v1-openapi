...
func Directory_resources_calendars_getHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateDirectory_resources_calendars_getTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_admin_directory_v1_customer_customer_resources_calendars_calendarResourceId",
		mcp.WithDescription("Retrieves a calendar resource."),
		...
		mcp.WithString("calendarResourceId", mcp.Required(), mcp.Description("The unique ID of the calendar resource to retrieve.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Directory_resources_calendars_getHandler(cfg),
	}
}
