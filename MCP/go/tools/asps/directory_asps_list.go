...
func Directory_asps_listHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/admin/directory/v1/users/%s/asps%s", cfg.BaseURL, userKey, queryString)
...
func CreateDirectory_asps_listTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_admin_directory_v1_users_userKey_asps",
		mcp.WithDescription("Lists the ASPs issued by a user."),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Directory_asps_listHandler(cfg),
	}
}
