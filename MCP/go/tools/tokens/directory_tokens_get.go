...
func Directory_tokens_getHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateDirectory_tokens_getTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_admin_directory_v1_users_userKey_tokens_clientId",
		mcp.WithDescription("Gets information about an access token issued by a user."),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Directory_tokens_getHandler(cfg),
	}
}
