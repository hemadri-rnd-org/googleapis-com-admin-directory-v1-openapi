...
func Directory_users_aliases_deleteHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/admin/directory/v1/users/%s/aliases/%s%s", cfg.BaseURL, userKey, alias, queryString)
...
func CreateDirectory_users_aliases_deleteTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_admin_directory_v1_users_userKey_aliases_alias",
	...
	return models.Tool{
		Definition: tool,
		Handler:    Directory_users_aliases_deleteHandler(cfg),
	}
}
