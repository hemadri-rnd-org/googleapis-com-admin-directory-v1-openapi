...
func Directory_users_aliases_watchHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/admin/directory/v1/users/%s/aliases/watch%s", cfg.BaseURL, userKey, queryString)
...
func CreateDirectory_users_aliases_watchTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_admin_directory_v1_users_userKey_aliases_watch",
	...
	return models.Tool{
		Definition: tool,
		Handler:    Directory_users_aliases_watchHandler(cfg),
	}
}
