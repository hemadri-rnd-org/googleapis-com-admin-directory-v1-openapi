...
func Directory_users_photos_getHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateDirectory_users_photos_getTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_admin_directory_v1_users_userKey_photos_thumbnail",
		mcp.WithDescription("Retrieves the user's photo."),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Directory_users_photos_getHandler(cfg),
	}
}
