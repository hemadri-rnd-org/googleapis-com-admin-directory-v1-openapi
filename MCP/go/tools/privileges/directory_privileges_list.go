...
func Directory_privileges_listHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/admin/directory/v1/customer/%s/roles/ALL/privileges%s", cfg.BaseURL, customer, queryString)
...
func CreateDirectory_privileges_listTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_admin_directory_v1_customer_customer_roles_ALL_privileges",
		mcp.WithDescription("Retrieves a paginated list of all privileges for a customer."),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Directory_privileges_listHandler(cfg),
	}
}
