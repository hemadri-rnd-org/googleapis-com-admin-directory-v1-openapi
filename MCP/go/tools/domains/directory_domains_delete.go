...
func Directory_domains_deleteHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/admin/directory/v1/customer/%s/domains/%s%s", cfg.BaseURL, customer, domainName, queryString)
...
func CreateDirectory_domains_deleteTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_admin_directory_v1_customer_customer_domains_domainName",
		mcp.WithDescription("Deletes a domain of the customer."),
		mcp.WithString("customer", mcp.Required(), mcp.Description("Immutable ID of the Google Workspace account.")),
		mcp.WithString("domainName", mcp.Required(), mcp.Description("Name of domain to be deleted")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Directory_domains_deleteHandler(cfg),
	}
}
