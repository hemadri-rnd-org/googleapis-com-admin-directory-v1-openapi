...
func Directory_domains_getHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateDirectory_domains_getTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_admin_directory_v1_customer_customer_domains_domainName",
		mcp.WithDescription("Retrieves a domain of the customer."),
		...
		mcp.WithString("domainName", mcp.Required(), mcp.Description("Name of domain to be retrieved")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Directory_domains_getHandler(cfg),
	}
}
