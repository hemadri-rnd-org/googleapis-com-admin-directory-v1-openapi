...
func Admin_channels_stopHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/admin/directory_v1/channels/stop%s", cfg.BaseURL, queryString)
...
func CreateAdmin_channels_stopTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_admin_directory_v1_channels_stop",
		mcp.WithDescription("Stops watching resources through this channel."),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Admin_channels_stopHandler(cfg),
	}
}
