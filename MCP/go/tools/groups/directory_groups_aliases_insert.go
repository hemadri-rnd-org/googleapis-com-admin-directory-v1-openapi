...
func Directory_groups_aliases_insertHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/admin/directory/v1/groups/%s/aliases%s", cfg.BaseURL, groupKey, queryString)
...
func CreateDirectory_groups_aliases_insertTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_admin_directory_v1_groups_groupKey_aliases",
		mcp.WithDescription("Adds an alias for the group."),
		...
		mcp.WithString("etag", mcp.Description("")),
		mcp.WithString("id", mcp.Description("")),
		mcp.WithString("kind", mcp.Description("")),
		mcp.WithString("primaryEmail", mcp.Description("")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Directory_groups_aliases_insertHandler(cfg),
	}
}
