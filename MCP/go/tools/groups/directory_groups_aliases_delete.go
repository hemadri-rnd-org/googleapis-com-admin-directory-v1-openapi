...
func Directory_groups_aliases_deleteHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/admin/directory/v1/groups/%s/aliases/%s%s", cfg.BaseURL, groupKey, alias, queryString)
...
func CreateDirectory_groups_aliases_deleteTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_admin_directory_v1_groups_groupKey_aliases_alias",
		mcp.WithDescription("Removes an alias."),
		...
		mcp.WithString("alias", mcp.Required(), mcp.Description("The alias to be removed")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Directory_groups_aliases_deleteHandler(cfg),
	}
}
