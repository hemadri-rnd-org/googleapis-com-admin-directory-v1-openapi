...
func Directory_chromeosdevices_getHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateDirectory_chromeosdevices_getTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_admin_directory_v1_customer_customerId_devices_chromeos_deviceId",
		mcp.WithDescription("Retrieves a Chrome OS device's properties."),
		...
		mcp.WithString("deviceId", mcp.Required(), mcp.Description("The unique ID of the device. The `deviceId`s are returned in the response from the [chromeosdevices.list](/admin-sdk/directory/v1/reference/chromeosdevices/list) method.")),
		mcp.WithString("projection", mcp.Description("Determines whether the response contains the full list of properties or only a subset.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Directory_chromeosdevices_getHandler(cfg),
	}
}
