...
func Directory_chromeosdevices_patchHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateDirectory_chromeosdevices_patchTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("patch_admin_directory_v1_customer_customerId_devices_chromeos_deviceId",
		mcp.WithDescription("Updates a device's updatable properties, such as `annotatedUser`, `annotatedLocation`, `notes`, `orgUnitPath`, or `annotatedAssetId`. This method supports [patch semantics](/admin-sdk/directory/v1/guides/performance#patch)."),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Directory_chromeosdevices_patchHandler(cfg),
	}
}
