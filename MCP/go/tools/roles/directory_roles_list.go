...
func Directory_roles_listHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/admin/directory/v1/customer/%s/roles%s", cfg.BaseURL, customer, queryString)
...
func CreateDirectory_roles_listTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_admin_directory_v1_customer_customer_roles",
		mcp.WithDescription("Retrieves a paginated list of all the roles in a domain."),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Directory_roles_listHandler(cfg),
	}
}
