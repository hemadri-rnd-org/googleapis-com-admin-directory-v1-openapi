...
func Directory_members_insertHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateDirectory_members_insertTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_admin_directory_v1_groups_groupKey_members",
		mcp.WithDescription("Adds a user to the specified group."),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Directory_members_insertHandler(cfg),
	}
}
