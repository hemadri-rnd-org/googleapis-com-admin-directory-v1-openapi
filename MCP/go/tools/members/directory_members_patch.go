...
func Directory_members_patchHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateDirectory_members_patchTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("patch_admin_directory_v1_groups_groupKey_members_memberKey",
		mcp.WithDescription("Updates the membership properties of a user in the specified group. This method supports [patch semantics](/admin-sdk/directory/v1/guides/performance#patch)."),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Directory_members_patchHandler(cfg),
	}
}
