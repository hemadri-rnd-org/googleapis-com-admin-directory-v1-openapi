...
func Directory_members_getHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateDirectory_members_getTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_admin_directory_v1_groups_groupKey_members_memberKey",
		mcp.WithDescription("Retrieves a group member's properties."),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Directory_members_getHandler(cfg),
	}
}
